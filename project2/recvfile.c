...
int main(int argc, char **argv) {
    ...
    char file_info[256];
    while (1) {
        ssize_t file_info_bytes = recvfrom(sock, file_info, sizeof(file_info) - 1, 0, (struct sockaddr *)&sender_addr, &addr_len);
        ...
        uint32_t calculated_crc = crc32((unsigned char *)recv_filename, strlen(recv_filename));
        ...
        if (calculated_crc == recv_crc) {
            ...
            if (sendto(sock, nack_buffer, strlen(nack_buffer) + 1, 0, (struct sockaddr *)&sender_addr, addr_len) < 0) {
    ...
    snprintf(output_filename, sizeof(output_filename), "%s/%s.recv", subdir, fname);
    ...
    while (1) {
        bytes_received = recvfrom(sock, buffer, sizeof(buffer), 0, (struct sockaddr *)&sender_addr, &addr_len);
        if (bytes_received < 0) {
        ...
        if (bytes_received != sizeof(int32_t) * 3 + data_length + sizeof(uint32_t)) {
            printf("[recv corrupt packet] Packet size mismatch. Expected: %zu, Received: %d\n",
        ...
        memcpy(&received_crc, buffer + sizeof(int32_t) * 3 + data_length, sizeof(uint32_t));
        received_crc = ntohl(received_crc);

        // Calculate CRC over the first 12 + data_length bytes
        uint32_t calculated_crc = crc32((unsigned char *)buffer, sizeof(int32_t) * 3 + data_length);
        ...
        //sliding window part
        if (seq_num >= window_start && seq_num < window_start + WINDOW_SIZE && acked[seq_num % WINDOW_SIZE]) {
            // duplicate
        ...
        if (expected_seq_num == seq_num){
            //in-order
            printf("[recv data] Seq_num: %d, Start: %d, Length: %d ACCEPTED[in-order]\n",seq_num,start_offset,data_length);
            size_t write_bytes = fwrite(buffer + sizeof(int32_t) * 3, 1, data_length, fp);
            printf("wrote %zu bytes to file.\n", write_bytes);

            expected_seq_num++;

            while(acked[expected_seq_num % WINDOW_SIZE]) {
                size_t write_bytes = fwrite(window[expected_seq_num % WINDOW_SIZE] + sizeof(int32_t) * 3, 1,
                                             length[expected_seq_num % WINDOW_SIZE], fp);
                printf("wrote %zu bytes to file.\n", write_bytes);
        ...
        else if(seq_num < window_start+WINDOW_SIZE && seq_num > expected_seq_num){
            //out-order
            printf("[recv data] Seq_num: %d, Start: %d, Length: %d ACCEPTED[out-of-order]\n",seq_num,start_offset,data_length);
