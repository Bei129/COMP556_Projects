...
int send_data_packet(int sock, struct sockaddr_in *recv_addr, socklen_t addr_len, 
                     char *send_buffer, char *data, int seq_num, int start_offset, int data_length) {
    ...
    uint32_t crc_value = crc32((unsigned char *)send_buffer, sizeof(int32_t) * 3 + data_length);
    uint32_t net_crc = htonl(crc_value);
    memcpy(send_buffer + sizeof(int32_t) * 3 + data_length, &net_crc, sizeof(uint32_t));

    if (sendto(sock, send_buffer, sizeof(int32_t) * 3 + data_length + sizeof(uint32_t), 0,
               (struct sockaddr *)recv_addr, addr_len) < 0) {
        perror("Error sending packet");
        return -1;
    }
    
    printf("[send data] Seq_num: %d, Start:%d, Length: %d, CRC: %u\n", seq_num, start_offset, data_length, crc_value);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    while (!eof || base < next_seq_num) {
        ...
        while (next_seq_num < base + WINDOW_SIZE && !eof) {
            int bytes_read = fread(window_data[next_seq_num % WINDOW_SIZE], 1, DATA_SIZE, fp);
            if (bytes_read <= 0) {
                eof = 1;
                // Send EOF packet
                if (send_data_packet(sock, &recv_addr, addr_len, send_buffer, NULL, -1, 0, 0) < 0) {
                    fclose(fp);
                    close(sock);
                    return 1;
                }
            ...
            if (send_data_packet(sock, &recv_addr, addr_len, send_buffer, window_data[next_seq_num % WINDOW_SIZE],
        ...
        if (select_retval == 0) {
            ...
            for (int i = base; i < next_seq_num; i++) {
                if (!acked[i % WINDOW_SIZE] && get_time_diff(send_times_arr[i % WINDOW_SIZE], current_time) >= timeout_interval) {
                    // Resend packet
                    if (send_data_packet(sock, &recv_addr, addr_len, send_buffer, window_data[i % WINDOW_SIZE],
                                         i, start[i % WINDOW_SIZE], window_data_length[i % WINDOW_SIZE]) < 0) {
                        fclose(fp);
                        close(sock);
                        return 1;
                    }
            ...
            ssize_t ack_bytes = recvfrom(sock, ack_buffer, sizeof(ack_buffer) - 1, 0, (struct sockaddr *)&ack_addr, &ack_addr_len);
            if (ack_bytes < 0) {
                perror("Error receiving ACK");
                fclose(fp);
                close(sock);
            ...
            if (ack_num >= base && ack_num < next_seq_num) {
                ...
                double sample_rtt = get_time_diff(send_times_arr[ack_num % WINDOW_SIZE], ack_time);
